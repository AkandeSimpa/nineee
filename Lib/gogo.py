"""
Scrapes the GoGo Anime website to retrieve information about available episodes, titles, and genres.

The `EpisodeScraper` class is responsible for scraping episode-specific information, such as the maximum number of episodes and generating links for individual episodes.

The `Goscraper` class is the main entry point for scraping data from the GoGo Anime website. It provides methods to retrieve a list of available titles, the number of results, pagination information, and a list of available genres.
"""
...
class Goscraper:
    ...
    def get_titles(self):
        ...
        #print("line 109 the titles are" + str(titles))
        #print("the episodes are " + str(episodes))
        #print("the flairs are " + str(flairs))
        scrape_res = []
        for (title, episode, flair) in itertools.zip_longest(titles, episodes, flairs) :
            ...
            print(str(scrape_data['flair']))
            get_image = self.get_image(flair=scrape_data["flair"])
            scrape_res.append(scrape_data)
            #print("the scrape data is " + str(scrape_data))
        return scrape_res
    
    

    def get_images(self):
        """Returns anime image URLs from titles"""
       
        
        titles = self.get_titles()
        images = []
        
        base_url = "https://gogocdn.net/cover/"
        
        for title in titles:
            flair = title["flair"]
            image_url = base_url + flair+".png"
            images.append(image_url)
            
        return images
    
    
    def get_image(self, flair):

        base_url = "https://gogocdn.net/cover/"

        if flair is not None:
            # Split flair on '/' and take last part
            split_flair = flair.split("/")[-1]  

            # Remove any 'category' prefix
            split_flair = split_flair.replace("category/", "")

            image_url = base_url + split_flair + ".png"
